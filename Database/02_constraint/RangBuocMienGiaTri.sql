...
-- 11. Bảng GIAODICHTHANHQUAN
ALTER TABLE GIAODICHTHANHQUAN ADD CONSTRAINT CHK_GD_PhuongThuc
    CHECK (PhuongThucTT IN ('Chuyển khoản', 'Thẻ tín dụng', 'Ví điện tử', 'Tiền mặt'));

ALTER TABLE GIAODICHTHANHQUAN ADD CONSTRAINT CHK_GD_TrangThai
    CHECK (TrangThaiGD IN ('Thành công', 'Thất bại', 'Đang xử lý', 'Hết thời gian'));

ALTER TABLE GIAODICHTHANHQUAN ADD CONSTRAINT CHK_GD_LanThuLai
    CHECK (LanThuLai >= 0);
...
ALTER TABLE LICHSUSOATVE ADD CONSTRAINT CHK_LSSV_KetQua
    CHECK (KetQuaQuet IN ('Hợp lệ', 'Vé giả', 'Vé đã sử dụng'));
